...
# 安全系数：单账号稍保守(1.3)，多账号有天然错峰可激进一点(1.2)
_SAFETY_SINGLE = 1.3
_SAFETY_MULTI = 1.2
...
def compute_dynamic_interval(endpoint: str) -> tuple[float, float, float]:
    ...
    Examples:
        单账号 search → (900/50/1*1.3)*0.8 ~ (900/50/1*1.3)*1.4  ≈ 18.7s ~ 32.7s
        3账号  search → (900/50/3*1.2)*0.8 ~ (900/50/3*1.2)*1.4  ≈ 5.8s ~ 10.1s
    """
    ...
    min_sec = safe_interval * 0.8
    max_sec = safe_interval * 1.4
