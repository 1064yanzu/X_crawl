"""
断点续爬检查点模块
将每次任务的爬取进度（游标 + 已采条数）持久化到 JSON 文件
服务重启后可从断点处继续爬取，不重新开始

性能优化：
1. checkpoint 只存增量游标，不存完整 tweets 列表（从 task_results 读取）
2. JSON 序列化和磁盘写入在后台线程池中异步执行，不阻塞爬虫主线程
3. 节流保存，避免每页都写盘
"""
import json
import logging
...
def _do_write(path: Path, data: dict, tweet_count: int, has_cursor: bool) -> None:
    ...
    try:
        ...
        logger.debug(
            f"检查点已保存: {path.name}（{tweet_count} 条推文，"
...
def save_checkpoint(
    ...
    keyword: str,
    product: str,
    tweets_count: int,  # 改为传入已采条数，不传完整列表
    next_cursor: Optional[str],
    page_fetched: int,
    extra: Optional[dict] = None,
) -> None:
    """
    保存断点到磁盘（异步非阻塞，只存游标和计数）

    JSON 序列化和磁盘 I/O 在后台线程池中执行，不阻塞调用方。
    完整 tweets 从 task_results 表读取，这里只存进度元数据。

    Args:
        task_id:       任务 ID
        keyword:       搜索关键词
        product:       搜索类型
        tweets_count:  已爬取推文数量
        next_cursor:   下一页 cursor（None 表示已爬完）
        page_fetched:  已爬取页数
        extra:         额外字段（如 fetch_replies 等）
    """
    ...
    data = {
        ...
        "keyword": keyword,
        "product": product,
        "tweets_count": tweets_count,  # 只存计数，不存完整列表
    ...
    _writer_pool.submit(_do_write, path, data, tweets_count, has_cursor)
...
def save_checkpoint_sync(
    ...
    if extra:
        data.update(extra)
    _do_write(path, data, tweets_count, bool(next_cursor))
...
def load_checkpoint(task_id: str) -> Optional[dict]:
    ...
    Returns:
        检查点数据 dict（只含游标和计数，完整 tweets 需从 task_results 读取）
        不存在则返回 None
    """
    path = _get_checkpoint_path(task_id)
    if not path.exists():
    ...
    try:
        ...
        if "tweets" in data and isinstance(data["tweets"], list):
            ...
            del data["tweets"]  # 移除大数据，节省内存

        logger.info(
            f"加载检查点: task_id={task_id}，"
            f"已有 {data.get('tweets_count', 0)} 条推文，"
...
def list_checkpoints() -> list[dict]:
    ...
    for p in _checkpoint_dir().glob("*.json"):
        try:
            ...
            if tweets_count is None and "tweets" in data:
