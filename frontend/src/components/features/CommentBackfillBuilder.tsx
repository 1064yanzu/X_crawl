...
function SummaryRow({ label, value }: { label: string; value: string }) {
    return (
        <div className="flex items-center justify-between gap-3 rounded-md border border-border bg-background px-3 py-3 text-sm">
            ...
            <div className="space-y-6 p-6 sm:p-7">
                <section className="space-y-4 rounded-lg border border-border bg-background p-5 shadow-sm">
                    ...
                    <div className="rounded-md border border-dashed border-border bg-muted/20 p-4">
                        <label className="flex cursor-pointer flex-col items-center justify-center gap-3 rounded-md border border-border bg-background px-5 py-8 text-center transition-colors hover:border-primary/30 hover:bg-background">
                        ...
                        {file ? (
                            <div className="mt-4 flex items-center gap-3 rounded-md border border-border bg-background px-4 py-3 text-sm">
                                <FileSpreadsheet className="h-4 w-4 text-primary" />
                                <div className="min-w-0 flex-1">
                                    <p className="truncate font-medium text-foreground">{file.name}</p>
                                    <p className="text-xs text-muted-foreground">{(file.size / 1024).toFixed(1)} KB</p>
                                </div>
                                <Button type="button" variant="ghost" size="sm" className="rounded-md" onClick={() => selectFile(null)}>
                                    重新选择
                                </Button>
                            </div>
                        ) : null}
                    ...
                    {platform === "x" ? (
                        <div className="grid gap-3 md:grid-cols-2">
                            <button
                                ...
                                className={cn(
 "rounded-md border px-4 py-4 text-left transition-all duration-200",
                                    replyDepth === 1 ? "border-primary/30 bg-primary/8 text-foreground shadow-sm" : "border-border bg-card hover:border-primary/20 hover:bg-muted/30",
                                )}
                            >
                            ...
                            <button
                                ...
                                className={cn(
 "rounded-md border px-4 py-4 text-left transition-all duration-200",
                                    replyDepth === 2 ? "border-primary/30 bg-primary/8 text-foreground shadow-sm" : "border-border bg-card hover:border-primary/20 hover:bg-muted/30",
                                )}
                            >
                            ...
                            <div className="space-y-2 md:col-span-2">
                                ...
                                <Input
                                    ...
                                    className="h-11 rounded-md bg-background font-mono"
                                />
                ...
                <section className="space-y-4 rounded-lg border border-border bg-background p-5 shadow-sm">
                    ...
                    {analyzing ? (
                        <div className="flex items-center gap-3 rounded-md border border-border bg-background px-4 py-4 text-sm text-muted-foreground">
                    ...
                    {analysisError ? (
                        <div className="rounded-md border border-red-200/70 bg-red-50/80 px-4 py-4 text-sm text-red-800 dark:border-red-500/20 dark:bg-red-500/10 dark:text-red-100">
            ...
            <aside className="border-t border-border bg-muted/15 p-6 xl:border-l xl:border-t-0 xl:p-7">
                <div className="space-y-4 xl:sticky xl:top-24">
                    <div className="rounded-lg border border-border bg-card p-5 shadow-sm">
                        ...
                        <Button onClick={() => void submit()} disabled={!canSubmit || submitting || analyzing} className="mt-6 h-11 w-full rounded-md">
