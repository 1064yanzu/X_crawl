...
- 新增 `POST /api/v1/comment-backfill/from-tasks`，支持从已完成帖子采集任务直接创建评论补采任务
- 支持默认补采到二级评论；批量提交多个历史任务时自动创建顺序队列
- 后端新增 `comment_backfill_task_source.py`，自动过滤已抓过评论、0 评论、缺关键字段和重复帖子
- 任务列表新增“批量补采评论”，任务卡片、右侧预览、任务详情新增“补采评论（2级）”快捷入口
- 补采按钮统一 loading 反馈，单任务创建后会直接跳转到新的评论补采任务详情页
...
**问题**：原时间分割策略固定使用 `window_days`（14天/7天），对于跨越数年的爬虫任务会产生上百个分段（如 5 年 ≈ 130-260 段），密集的请求容易触发 X 的反爬机制。

**改进**：
- `x_time_splitter.py` 引入**自适应窗口**策略：
  - 跨度 < 90 天：保持原有行为，使用用户配置的 `window_days`（如 14 天或 7 天）
  - 跨度 ≥ 90 天：自动升级为按**自然月**分割（每段 ~28-31 天），分段数大幅减少
- 新增 `_split_by_calendar_month()` 函数，按自然月边界（如 1月15日→2月15日→3月15日）分割
- 新增 `_split_by_fixed_days()` 函数，原有的固定天数分割策略独立出来
- 新增 `_add_one_month()` 工具函数，正确处理月末边界（如 1-31→2-28/29）
- 前端 `CrawlerTaskBuilder.tsx` 提示文案优化，长跨度时明确告知"按月分割"
- 前端 `CrawlerConfigCard.tsx` 配置项描述更新，标注 ≥90 天时配置不生效
- 测试文件完全重写，覆盖短跨度、长跨度、多年跨度、max_segments 上限、月中起点等 8 个场景

**效果对比**（以 5 年跨度为例）：
| 场景 | 旧策略 | 新策略 |
|------|--------|--------|
| 限定模式（window=14天）| ~130 段 | 60 段（按月）|
| 无上限模式（window=7天）| ~260 段 | 60 段（按月）|
...
**修复**（`searcher.py`）：
- 用 `try/finally` 包裹整个搜索逻辑（登录验证 → Cookie 准备 → 分页搜索 → 评论抓取），`finally` 块中确保 `tab.close()` 被调用
- 无论搜索正常完成、被中断（`StopSignal`）还是异常退出，搜索 tab 都会被正确关闭

## 2026-03-05

### ✨ 更新：拉取远程仓库最新代码

**背景**：用户需要获取最新的远程仓库代码并覆盖本地工作区。
**操作**：执行了 `git fetch origin` 与 `git reset --hard origin/main` 更新并覆盖本地代码。


### ✨ 新增：微博子评论（楼中楼）深度抓取

**背景**：微博评论 API 的 `total_number` 包含所有层级评论，但 `data` 数组只返回顶层评论，每条顶层评论的 `comments` 字段仅内嵌部分子评论（通常 2~3 条）。

**新增**（`sub_comment_fetcher.py` + `comment_fetcher.py` 集成）：
- 新增 `sub_comment_fetcher.py` 模块：使用 `fetch_level=1` + `id={评论ID}` 参数获取任意顶层评论的完整子评论列表（支持分页）
- `comment_fetcher.py` 在翻页完成后增加**阶段 3：子评论补全**，使用 `total_number` + `more_info.text` 双重判断精准筛选需要补全的评论（实测 319 条中仅 5 条需要额外请求，避免触发风控）
- **前端实时进度**：全流程（首页拦截 → 翻页 → 二级评论补全）通过 `update_task_phase` 推送到 SSE 流

### 🐛 修复：微博评论"只抓到几十条"（API 数据结构理解错误 + 翻页机制重写）

**问题**：帖子显示 448 条评论，但实际只抓到 22 条，看似翻页失败。
**根因**（经过 v3→v6 共 4 轮排查定位到真正原因）：
1. ~~v3 DOM 桥接方案死锁~~ ~~v4 滚动触发干扰~~ ~~v5 缺少请求头~~
2. **真正原因**：API 的 `total_number=448` 包含**所有层级**的评论（顶层+子评论+子子评论），而 `data` 数组只返回顶层评论。每条顶层评论的 `comments` 字段内嵌部分子评论。该帖子顶层评论只有 22 条，**API 确实已全部返回**
3. 辅因：翻页方式从 JS fetch（缺少 `client-version` 等头导致降级响应）改为 Python `requests` 直接请求

**修复**（`comment_fetcher.py` v6）：
- **正确理解 API 结构**：`total_number` 是全层级评论总数，不再作为翻页完成度的参考
- **翻页改为 Python requests**：从 tab 提取 Cookie → `requests.Session` 直接请求 API → 完整的请求头对照抓包还原
- **正确统计评论数**：新增 `_count_all_comments` 递归统计顶层+子评论，`_parse_and_collect` 统一解析去重
- **日志区分两类计数**：明确显示"顶层 22 条（含 9 条子评论）"而非误导性的"22/448 条"
- **提取 Cookie 后关闭 tab**：释放浏览器资源

### ✨ 重构：微博评论抓取改为混合模式（反爬增强 + 翻页可靠性）

**问题**：
1. v1（JS fetch 注入）：在 `s.weibo.com` 域名下发请求 → 缺少正确 Referer/上下文 → 被反爬识别 → 评论抓不全
2. v2（纯网络拦截+滚动）：headless 模式下滚动无法触发页面内置 JS 的评论翻页请求 → 首页拦截到后翻页超时 → 仍然只有几十条

**方案**（v3 混合模式）：
- **首页**：通过 `tab.listen` 网络拦截（真实页面加载自动触发的评论请求，完全模拟人类行为）
- **后续翻页**：在帖子页面上下文中使用 JS fetch 主动请求（same-origin，Cookie/XSRF-TOKEN/Referer 都自然正确）

**修改**：
- `comment_fetcher.py`（**v3 重写**）：
  - 阶段1：新开标签页 → `tab.listen.start()` → 导航帖子页面 → 拦截首批评论
  - 阶段2：在帖子页面上下文中 JS fetch 后续分页（DOM 桥接方案读取结果）
  - 每页间模拟滚动增加自然性
- `searcher.py`：
  - 传入 `post_url=post.url`，移除 `tab` 参数
  - 移除评论抓取后导航回搜索域名的逻辑
